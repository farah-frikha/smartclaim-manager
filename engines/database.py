...
-- Couche 4 – Authentification & Utilisateurs
...
def lister_dossiers(statut: str = None, limite: int = 50) -> list:
    ...
    try:
        ...
        conn.close()
# =========================================================
# SECTION UTILISATEURS — CRUD authentification
# =========================================================
...
def mettre_a_jour_derniere_connexion(utilisateur_id: int) -> None:
    """Met à jour l'horodatage de dernière connexion."""
    conn = get_connection()
    try:
        conn.execute("""
            UPDATE utilisateurs
...
def lister_utilisateurs(role: str = None) -> list:
    """
    Liste les utilisateurs, filtre optionnel par rôle.
    ...
    try:
        if role:
            rows = conn.execute("""
                SELECT utilisateur_id, email, role,
                       nom_complet, actif, derniere_connexion, created_at
                FROM utilisateurs
                WHERE role = ?
